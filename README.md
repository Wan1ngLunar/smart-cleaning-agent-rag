...
真实模型冒烟评估为3/3通过，60条全量端到端评估为59/60通过：35/36条正例正常回答并附带来源，24/24条负例全部拒答。唯一失败用例`low_battery_immediate_recharge`的正确证据未进入Top-10，模型在资料不足时安全拒答；该结果作为后续查询改写、混合检索和重排序的固定优化样本保留，而不是通过放宽拒答规则或修改问题规避。上述命令会调用真实Chroma和DashScope，不属于默认CI，运行前需要配置有效API Key，并会产生API用量。
