...
当前基线为 33 项测试通过，核心模块测试覆盖率为 71.28%。测试覆盖来源去重、PDF 页码、混合拒答、检索指标、SQLite表与安全序列化、跨Agent实例恢复、会话隔离、历史消息过滤和UUID校验。现有一条 `langchain-community` 弃用警告，已记录为后续依赖迁移事项，没有通过过滤规则隐藏。
