...
4. 知识问答场景会检索 Chroma 中的本地文档，由模型生成带编号引用的回答，再由服务层追加去重后的文件名和 PDF 页码。
...
当前基线为 15 项测试通过，核心模块测试覆盖率为 68%。其中 RAG 服务覆盖率为 97%，已验证来源去重、PDF 页码、空检索和空模型回答。现有一条 `langchain-community` 弃用警告，已记录为后续依赖迁移事项，没有通过过滤规则隐藏。
