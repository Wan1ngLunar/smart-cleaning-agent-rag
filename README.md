...
当前基线为41项测试通过，核心模块测试覆盖率为77.57%。测试覆盖来源去重、PDF页码、本地UTF-8文本与PDF加载、混合拒答、检索指标、SQLite表与安全序列化、跨Agent实例恢复、会话隔离、历史消息过滤、UUID校验、请求编号、模型与工具耗时日志、异常包装和日志参数脱敏。模型和文件加载代码已移除`langchain-community`及旧`dashscope` SDK，测试不再产生对应的弃用警告。
