...
- **混合检索与重排序**：使用中文双字分词、BM25和向量召回，通过RRF融合候选，再由qwen3-rerank选出最终Top-3；提供30组参数搜索和统一A/B评测脚本。
...
5. Agent将用户问题交给模型，由模型判断是否需要调用工具；知识问答场景分别执行Chroma向量召回和BM25关键词召回，再通过RRF融合10条候选并由qwen3-rerank选出Top-3。
6. 向量侧使用最低相关性分数过滤明显无关片段；重排序后的证据继续由System消息核对问题对象、范围和时效，资料不足时统一拒答，资料充分时生成带编号引用的回答并追加文件名和PDF页码。
...
打开 `.env`，将占位符替换为自己的DashScope API Key和百炼业务空间重排序地址：

```dotenv
DASHSCOPE_API_KEY=你的真实密钥
DASHSCOPE_RERANK_BASE_URL=https://你的业务空间ID.cn-beijing.maas.aliyuncs.com/compatible-api/v1
```

聊天与向量模型使用DashScope的`compatible-mode/v1`地址；qwen3-rerank使用业务空间专用的`compatible-api/v1`地址，两者不是同一个接口路径。
...
当前基线为127项测试通过，Agent、FastAPI后端、HTTP前端客户端及核心模块总覆盖率为83.62%。测试覆盖API数据契约、生命周期、健康检查、聊天与历史路由、安全错误、HTTP连接异常、前后端导入边界、容器服务与密钥隔离，以及RAG、BM25、RRF融合、重排序、查询改写实验、引用过滤、工具路由与Token统计、降级路径、延迟初始化、资源释放、SQLite会话、请求追踪和60条评测集守卫。开发测试使用`httpx2`适配当前Starlette TestClient，生产前端继续使用固定版本`httpx`，完整测试不产生对应的弃用警告。
...
# 对比纯向量、BM25与RRF混合召回以及qwen3-rerank重排序指标。
python -m evaluation.compare_reranking

# 对4条固定困难用例比较原查询与模型改写查询；该实验不改变生产链路。
python -m evaluation.compare_query_rewriting
...
# 评估12条首步工具选择，并统计该次路由模型调用的Token分布。
python -m evaluation.evaluate_tool_routing
```
...
混合检索使用中文双字词元BM25补充余弦向量召回，再通过Chroma文档ID执行RRF去重融合。对向量候选`10/20`、BM25候选`5/10/20`和RRF常数`1/5/10/30/60`进行30组网格搜索后，推荐配置为向量候选10、BM25候选20、RRF常数10。人工复核并补全一个有效来源标注后，36条正例的Hit@1由80.56%提升到88.89%，Hit@3由94.44%提升到97.22%，MRR由0.8750提升到0.9306。

生产RAG在10条融合候选上使用qwen3-rerank，并保留最终Top-3。混合候选Recall@10为100%；重排序后Hit@1为88.89%、Hit@3为100%、MRR为0.9398，相比混合Top-3分别变化`+0.00`、`+2.78`个百分点和`+0.0093`。重排序接口异常时安全降级到RRF结果，不因外部排序服务暂时不可用而中断整个知识问答。

查询改写针对重排序后仍非Top-1的4条固定困难用例进行了定向A/B实验：改写查询仅用于向量与BM25召回，最终重排序仍使用用户原问题。原查询与改写查询的Hit@1均为0/4、MRR均为0.4583，结果为0条提升、0条退化、4条不变。由于现有问题表达已经清晰且混合候选Recall@10达到100%，同义改写没有补充新的召回信息，因此生产链路不增加查询改写调用。
...
重排序接入后的真实模型冒烟评估为3/3通过，60条全量端到端评估为60/60通过：36/36条正例正常回答并附带来源，24/24条负例全部拒答。固定困难用例`low_battery_immediate_recharge`的20%直接证据由RRF第6提升到重排序第1；完整HEPA维护周期证据由RRF第4提升到重排序第1。上述真实评测会访问Chroma和DashScope，不属于默认CI，运行前需要有效API Key和业务空间重排序地址，并会产生API用量。

全量端到端评测同时记录真实请求耗时与引用：行为准确率100%，36条正例预期来源引用命中率100%，正文实际引用与当前人工来源标注一致率78.18%，最近排名法P95响应时间为6779.18毫秒。来源列表只展示正文实际使用的编号，不再暴露未被正文引用的候选片段；78.18%表示与尚未穷举全部辅助来源的人工标注一致程度，不能直接解释为引用错误率。

12条真实首步工具路由用例全部通过，工具路由成功率为100%。单次首步路由模型调用平均消耗2184.75 Token，P95为2407 Token，其中平均输入2117.00、平均输出67.75。该Token统计只覆盖模型选择起始工具的调用，不包含后续Embedding、重排序、工具执行或最终回答生成，因此不冒充完整请求成本。
...
- **连接生命周期**：后端进程复用一个ReactAgent和SQLite连接，RAG服务在首次工具调用时延迟创建并复用重排序HTTP连接池；FastAPI退出时统一关闭这些资源，每个浏览器会话复用独立前端HTTPX连接池。
...
- **重排序可降级**：qwen3-rerank依据问题与片段的直接问答相关性重排融合候选；接口失败时退回RRF Top-3，避免把外部重排序服务变成单点故障。
...
- 当前检索指标主要按预期来源文件统计，仍可能出现“文件名正确但片段不含完整答案”的来源级命中；低电量和HEPA固定困难样本已通过片段内容人工复核，但尚未为全部正例建立自动化片段级证据标注。
...
- 将评测集继续扩充到100条左右并引入真实问法；在已有引用、工具路由、P95和首步路由Token指标基础上，补充片段级证据标注与完整请求Token/成本采集。
- 收集真实用户中的省略指代、口语短问句和多轮上下文依赖问题；只有新数据证明查询改写存在净收益时再重新评估生产接入。
